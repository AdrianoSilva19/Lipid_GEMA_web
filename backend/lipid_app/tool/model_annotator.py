...
class LipidNameAnnotator:
    ...
    def treat_data(self, dict_list: List[tuple]):
        ...
        for (
            ...
            self.counter = {
                i: self.counter.get(i, 0) + counter.get(i, 0)
                for i in set(self.counter).union(counter)
            }
            self.converted_lipid_class_dict = {
                i: self.converted_lipid_class_dict.get(i, 0)
                + converted_lipid_class_dict.get(i, 0)
                for i in set(self.converted_lipid_class_dict).union(
                    converted_lipid_class_dict
                )
            }
    ...
    def find_model_lipids(
        self, model: Model, n_jobs: int = 6
    ) -> Tuple[Dict, Dict, Dict, Dict, Model]:
        """
        ...
        resultados = parallel_callback(
            delayed(self._find_model_lipids)(model.metabolites[i])
    ...
    def search_lipid_synonyms(
        self, metabolite: Metabolite, side_chain: List[str]
    ) -> Tuple[Dict, Dict]:
        """
        ...
        if get_synonym is not None and not get_synonym[2]:
            backbone_id, sidechain_id, compound = (
                get_synonym[0],
                get_synonym[1],
                get_synonym[2],
            )
            if False not in sidechain_id:
                for backbone in backbone_id:
                    ...
                    if get_compound is not None and get_compound[0] == 0:
                        ...
                        results, counter = self.get_id_from_compound_with_no_backbone(
                            backbone, side_chains_compound, metabolite, results, counter
                        )
        ...
        if get_synonym is not None and get_synonym[2]:
            ...
            if False not in sidechain_id:
                results, counter = self.get_id_from_compound_entity(
                    backbone_id, sidechain_id, compound, metabolite
                )

        return counter, results

    def get_id_from_compound_with_no_backbone(
        self,
        backbone: int,
        side_chains_compound: List,
    ...
    def get_id_from_compound_entity(
        self,
        backbone_ids: List,
        sidechain_id: List,
        compound: bool,
    ...
    def get_synonym_id(self, backbone: str, side_chain: list) -> tuple:
        ...
        backbone_node = session.run(
            "match (s:Synonym) where s.synonym=$backbone return ID(s) as boimmg_id",
